const components = {
  header: {
    common: {
      openAccountMenu: "Open account menu",
      toggleSidebar: "Toggle sidebar",
      closeTab: "Close tab",
    },
    theme: {
      system: "System",
      light: "Light",
      dark: "Dark",
    },
    me: {
      fallbackName: "User",
      uploadAvatar: "Upload Avatar",
      settings: "Settings",
      profile: "Profile",
      logout: "Logout",
      crop: {
        title: "Adjust Avatar",
        description: "Drag and zoom. The circle is the visible avatar area.",
        zoom: "Zoom",
        cancel: "Cancel",
        confirm: "Save",
        confirming: "Uploading...",
      },
    },
  },
  calendar: {
    previousYearLabel: "Go to previous year",
    previousMonthLabel: "Go to previous month",
    nextMonthLabel: "Go to next month",
    nextYearLabel: "Go to next year",
  },
  copyableText: {
    copyLabel: "Copy",
    copiedLabel: "Copied",
  },
  avatarUpload: {
    select: "Upload Avatar",
    uploading: "Uploading...",
    errors: {
      imageOnly: "Please choose an image file.",
    },
    crop: {
      title: "Adjust Avatar",
      description: "Drag and zoom. The circle is the visible avatar area.",
      zoom: "Zoom",
      cancel: "Cancel",
      confirm: "Save",
      confirming: "Uploading...",
    },
  },
  dataTable: {
    emptyText: "No data available.",
    errorText: "Failed to load data.",
    loadingText: "Loading data...",
    refreshLabel: "Refresh data",
    resetLabel: "Reset filters",
    totalLabel: "Total",
    insertLabel: "Insert",
    actionsLabel: "Actions",
    editLabel: "Edit",
    deleteLabel: "Delete",
    moreLabel: "More actions",
    cancelLabel: "Cancel",
    saveLabel: "Save",
    confirmDeleteLabel: "Delete",
    deleteDialogTitle: "Confirm deletion",
    deleteDialogDescription: "This action cannot be undone.",
    sortAscendingLabel: "Sort ascending",
    sortDescendingLabel: "Sort descending",
    clearSortLabel: "Clear sort",
    bulkUpdateTitle: "Bulk Update",
    bulkUpdateFieldLabel: "Field",
    bulkUpdateValueLabel: "Value",
    bulkUpdateCancelLabel: "Cancel",
    bulkUpdateApplyLabel: "Apply",
    createdAtLabel: "Created At",
    updatedAtLabel: "Updated At",
    auditQueryLabel: "Time",
    auditFieldPlaceholder: "Time Field",
    auditRangePlaceholder: "Select Date Range",
    createdAtRangePlaceholder: "Select Created At",
    updatedAtRangePlaceholder: "Select Updated At",
    auditEmptyText: "-",
  },
  datePicker: {
    placeholder: "Select date",
    clearLabel: "Clear date",
  },
  dateRangePicker: {
    placeholder: "Select date range",
    endDate: "End date",
    clearLabel: "Clear date range",
  },
  dateTimePicker: {
    placeholder: "Select date time",
    clearLabel: "Clear date time",
    timeTitle: "Time",
    timeAria: "Date time picker time",
    now: "Now",
    today: "Today",
    yesterday: "Yesterday",
    weekStart: "Week Start",
    monthStart: "Month Start",
    yearStart: "Year Start",
  },
  multipleDatePicker: {
    placeholder: "Select multiple dates",
    modeLabel: "Multi",
    selectedDaysSuffix: "days selected",
  },
  timePicker: {
    placeholder: "Select time",
    clearLabel: "Clear time",
    ariaLabel: "Time",
  },
} as const

export default components
