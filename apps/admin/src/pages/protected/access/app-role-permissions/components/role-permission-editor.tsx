        ...
        <div className="grid min-w-0 gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 text-sm text-(--app-muted-text)">
              {t("admin.access.appRolePermissions.editor.codePrefix")}
              {activeRole.code}
              {t("admin.access.appRolePermissions.editor.descriptionSuffix")}
            </p>
          </div>
          <div className="flex flex-wrap justify-start gap-2">
            <Button variant="outline" onClick={onReset} disabled={isTreeLoading}>
              {t("admin.access.appRolePermissions.editor.reset")}
            </Button>
            <Button
              variant="primary"
              onClick={onSave}
            ...
            >
              {t("admin.access.appRolePermissions.editor.save")}
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        <div className="w-full max-w-80">
          <SearchInput
            ...
            placeholder={t(
              "admin.access.appRolePermissions.editor.searchPlaceholder"
            )}
          />
        </div>
        ...
        <div className="rounded-(--ui-radius-md) bg-(--app-panel) p-3">
          <TreeView
            ...
            emptyLabel={
              isTreeLoading
                ? t("admin.access.appRolePermissions.editor.loading")
                : t("admin.access.appRolePermissions.editor.empty")
            }
          />
        </div>
      </div>
